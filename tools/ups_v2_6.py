...
def run_workflow_logic(zip_file, excel_df, ship_key, cart_key, duplicate_carton=False):
    """核心处理逻辑：重命名 -> 排序 -> 合并"""
    log = []
            ...
            if st.button("开始自动化处理", type="primary"):
                with st.spinner("处理中..."):
                    try:
                        ...
                        pdf_bytes, logs = run_workflow_logic(zip_f, excel_df, s_key.lower(), c_key.lower(), duplicate_carton)
